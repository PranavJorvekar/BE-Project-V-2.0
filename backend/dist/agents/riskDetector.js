...
// ─── Mock Risk Detection ──────────────────────────────────────────────────────
function mockRisks(tasks, team, epics) {
    const warnings = [];
    ...
    if (team.length > 0) {
        ...
        const overloaded = Object.entries(taskCounts).find(([, count]) => count / tasks.length > 0.5);
        if (overloaded) {
            warnings.push({
                severity: "Critical",
    ...
    const totalCapacityHours = team.reduce((sum, m) => sum + m.weeklyHours * (epics.length > 0 ? epics.reduce((s, e) => s, 0) / team.length : 8), 0);
...
// ─── Real LLM ────────────────────────────────────────────────────────────────
async function realRisks(tasks, team, epics) {
...
// ─── Exported Agent ──────────────────────────────────────────────────────────
async function riskDetector(tasks, team, epics) {
